...
- `overrides-route.ts`: GET/POST/DELETE for overrides. Uses cookie fallback auth for browser-originated calls from calendar UI (`credentials: include`) while keeping Bearer support.
...
- `members-route.ts`: members API (`/api/members`, GET/PATCH). PATCH updates own `working` flag.
