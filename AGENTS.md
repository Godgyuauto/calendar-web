...
- Core Engine: Pure TypeScript (`lib/shift-engine.ts`)
...
- Public API는 `@/modules/shift`에서 import. `lib/shift-engine.ts`는 기존 호출부 호환용 1줄 re-export만 유지.
...
├── lib/                            # 호환 re-export 레이어 (각 파일 1줄)
│   ├── shift-engine.ts  → @/modules/shift
│   ├── calendar-grid.ts → @/modules/calendar
│   └── family-store.ts  → @/modules/family
├── supabase/
...
2. **폴더별 문서 위치**:
   ...
   - `lib/` — compat re-export 전용임을 명시 (신규 코드는 `@/modules/*`로 직접 import)
...
4. **수정 경계**: 폴더 문서의 "agent safe edit guide" 항목을 벗어나는 수정은 사용자 승인을 받아야 합니다. 특히 `lib/` 하위 파일을 두껍게 만들지 마세요 — 로직은 반드시 `@/modules/*`로.
